...
class contrastive_loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # loss = self.positive_weight * target * torch.pow((1 - x), 1) + (1 - target) * torch.pow(f1(x - 0.5), 1)
        loss = self.positive_weight * target * torch.pow(f1(0.5 - x), 2) + (1 - target) * torch.pow(x, 2)
